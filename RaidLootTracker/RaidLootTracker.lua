...
local function Announce(msg)
    local ch=RLT.db and RLT.db.channel or "RAID"
    if ch=="RAID_WARNING" then
        if IsInRaid() then SendChatMessage(msg,"RAID_WARNING")
        elseif IsInGroup() then SendChatMessage(msg,"PARTY")
        else SendChatMessage(msg,"SAY") end
    elseif IsInRaid()      then SendChatMessage(msg,ch)
    elseif IsInGroup() then SendChatMessage(msg,"PARTY")
    else Print(msg) end
end
...
local function BuildUI()
    ...
    HDivider(sp,-30)
    ColHeader(sp,"Player",8,-36,160); ColHeader(sp,"+1s",172,-36,44,"CENTER"); ColHeader(sp,"",220,-36,30); ColHeader(sp,"Bar",258,-36,130)
    HDivider(sp,-48)
    ...
    for i=1,STAND_ROWS do
        ...
        local namFS=FS(8,160); local cntFS=FS(172,44,"CENTER")
        namFS:SetTextColor(C.ui.text[1],C.ui.text[2],C.ui.text[3]); cntFS:SetTextColor(C.ui.orange[1],C.ui.orange[2],C.ui.orange[3])
        -- +/- buttons
        local btnPlus =MakeButton(sp,"+",18,16,C.ui.green[1],C.ui.green[2],C.ui.green[3])
        local btnMinus=MakeButton(sp,"-",18,16,C.ui.red[1],  C.ui.red[2],  C.ui.red[3])
        btnPlus:SetPoint("TOPLEFT",sp,"TOPLEFT",220,yB-1)
        btnMinus:SetPoint("LEFT",btnPlus,"RIGHT",2,0)
        local barBg=BgTex(sp,0.14,0.14,0.14,1,"ARTWORK"); barBg:SetHeight(8); barBg:SetWidth(130); barBg:SetPoint("TOPLEFT",sp,"TOPLEFT",258,yB-5)
        ...
        for _,v in ipairs({bg,namFS,cntFS,btnPlus,btnMinus,barBg,barFill}) do v:Hide() end
        local row={bg=bg,name=namFS,count=cntFS,btnPlus=btnPlus,btnMinus=btnMinus,barBg=barBg,barFill=barFill,fullName=nil}
        btnPlus:SetScript("OnClick",function()
            if not row.fullName then return end
            SetPlusOnes(row.fullName, GetPlusOnes(row.fullName)+1)
            UI.RefreshStandings()
        end)
        btnMinus:SetScript("OnClick",function()
            if not row.fullName then return end
            local cur=GetPlusOnes(row.fullName)
            SetPlusOnes(row.fullName, math.max(0,cur-1))
            UI.RefreshStandings()
        end)
        standRows[i]=row
    end
    ...
    function UI.RefreshStandings()
        ...
        for i,row in ipairs(standRows) do
            ...
            if e then
                ...
                local pct=e.count/maxC; row.barFill:SetWidth(math.max(2,130*pct))
                local r=math.min(1,pct*1.5); local g=math.max(0,1-pct)
                row.barFill:SetColorTexture(r,g*0.6,0.05,1)
                for _,v in ipairs({row.bg,row.name,row.count,row.btnPlus,row.btnMinus,row.barBg,row.barFill}) do v:Show() end
            else
                row.fullName=nil
                for _,v in ipairs({row.bg,row.name,row.count,row.btnPlus,row.btnMinus,row.barBg,row.barFill}) do v:Hide() end
            end
        end
    end
    ...
    local chanBtns,CHANS={},{"RAID","PARTY","SAY","RAID_WARNING"}
    local CHAN_LABELS={RAID="RAID",PARTY="PARTY",SAY="SAY",RAID_WARNING="RW"}
    local cRight=-8
    for i=#CHANS,1,-1 do
        local ch=CHANS[i]
        local lbl=CHAN_LABELS[ch] or ch
        local btn=MakeButton(setp,lbl,52,BTN_H,C.ui.accent[1],C.ui.accent[2],C.ui.accent[3])
